...
float findInterEllipseCycle(Point2 h1,Point2 h2,Point2 c1,int ah,int h3,float avgHopDis1,float avgHopDis2,vector<float>&  );
...
vector<Point2> mergeClosePoints(vector<Point2> &vp){

  vector<Point2> res;
  for(vector<Point2>::iterator it = vp.begin(); it != vp.end(); ++it){
    bool flag = true;
		for(vector<Point2>::iterator it2 = it; it2 != vp.end(); ++it2){
		  if(it!=it2 && euDis2((*it).m_X,(*it).m_Y,(*it2).m_X,(*it2).m_Y)<5){
...
float findInterEllipseCycle(Point h1,Point h2,Point c1,int ah,int h3,float avgHopDis1,float avgHopDis2,vector<float>& yr )
{
  ...
  if(c<10){
    ...
    double tempArea=circleOverlapArea(tempCircleCenter, ah, c1, h3, avgHopDis1);
    for( int jjj = 0;jjj<2;jjj++)
    {
    
      yr.push_back(tempArea);
    }
    return tempArea;
  }
  ...
  Point2 orig = {0.0,0.0};
	...
	else if (vsize ==3)
	{
      ...
      float aa4 = getCircleSegArea(result[1],result[0],newOrig, r);
	    double area1 = min(aa1,aa2);
	    area1 += min(aa3,aa4);
	    float ab1 = getEllipseSegArea(result[0],result[2],orig,a,b);
      float ab2 = getCircleSegArea(result[0],result[2],newOrig, r);
      float ab3 = getEllipseSegArea(result[2],result[0],orig,a,b);
      float ab4 = getCircleSegArea(result[2],result[0],newOrig, r);
	    double area2 = min(ab1,ab2);
	    area2 += min(ab3,ab4);
	    float ac1 = getEllipseSegArea(result[1],result[2],orig,a,b);
      float ac2 = getCircleSegArea(result[1],result[2],newOrig, r);
      float ac3 = getEllipseSegArea(result[2],result[1],orig,a,b);
      float ac4 = getCircleSegArea(result[2],result[1],newOrig, r);
	    double area3 = min(ac1,ac2);
	    area3 += min(ac3,ac4);
	    area+=min(area3, min(area2,area1));
	}  
	else{
...
*/		    
		    ...
		    for( int i =0;i<4;i++){
		      ...
		      if(((abs(cx)>1 && abs(solutions[i].imag())<0.1)||(abs(cx)<=1 && abs(solutions[i].imag())<1)) && (b*b-res*res)>=-1 && (r*r-(res-cy)*(res-cy))>=-1){
		        temp.push_back(res);
		        numberOfRoot++;
		    ...
		    for(vector<double>::iterator it = temp.begin(); it != temp.end(); ++it){
		      ...
		      if(numberOfRoot == 1 && v1<100 && v2<100){
		        Point2 tempPoint = {res1, y};
            result.push_back(tempPoint);
            Point2 tempPoint2 = {res2, y};
            result.push_back(tempPoint2);
            break;
		      ...
		      if(abs(res1)<5){
		          res1 = 0;
		          res2 = 0;
		          if(v1<(1000/(1+abs(res1)))||v2<(1000/(1+abs(res1))))
		          {
                Point2 tempPoint = {res1, y};
                result.push_back(tempPoint);
		          }
		    ...
		    sortWithAngle(result);
		    result2=mergeClosePoints(result);
